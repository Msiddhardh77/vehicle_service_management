...
def update_status(request_id):
    if session.get('admin'):
        ...
        email = cursor.fetchone()[0]
        subject="vehicle service request"
        body=f"{cid[1]} of your vehicle service request is {new_status} - date of requested {cid[2]}"
        sendmail(to=email,subject=subject,body=body)
        cursor.close()
...
def update_job(request_id):
    if session.get('admin'):
        ...
        subject="vehicle service request"
        body=f"{email[1]} of your job application status is {new_status}"
        sendmail(to=email[0],subject=subject,body=body)
        flash('Service request submitted successfully.')
